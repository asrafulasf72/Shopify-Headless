...
export function HeaderMenu({
  ...
  const baseClassName = "transition-all duration-200 hover:text-yellow-600 relative after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[1px] after-bg-yellow-600 after:transition-all after:duration-300 hover:after:w-full ";
    ...
    <nav className={viewport === 'desktop' ? desktopClassName : mobileClassName} role='navigation'>
        ...
        <>
          ...
          <div className='space-y-6 py-4'>
            {menu?.items.map((item) => {
              ...
              return (
                <NavLink className={({ isActive }) => `${baseClassName} text-lg py-2 block ${isActive ? "text-yellow-600" : " text-blue-950"}`} key={item.id} onClick={close} prefetch='intent' to={url}>
                  {item.title}
                </NavLink>
              )
            })}
      ...
      {viewport === 'desktop' && (
        ...
        menu?.items.map((item) => {
          ...
          const url = item.url.includes("myshopify.com") || item.url.includes(publicStoreDomain) || item.url.includes(primaryDomainUrl) ? new URL(item.url).pathname : item.url;
          return (
            <NavLink className={({ isActive }) => `${baseClassName} ${isActive ? "text-yellow-600" : " text-blue-950"}`} key={item.id} onClick={close} prefetch='intent' to={url}>
              {item.title}
            </NavLink>
          )
        })
